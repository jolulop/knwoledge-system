#!/usr/bin/env python3
"""Human-add producer path (ADR-0058): a reviewer adds a concept/entity the extractor missed.

The POST is PRODUCER-side work — the same mutation class as `extract_concepts`, which was
...
Duplicate/identity routing (identity is name-hashed and frozen, ADR-0021 — never mint a
second node for a known name): existing candidate/tombstone -> add the mention and
approve/reuse its promote item; already active -> mention only, no promote item;
entity-family subtype collision -> mention routed to the existing node + a
`change_entity_subtype` item (rekey = its own risk class, ADR-0051); concept<->entity
cross-family conflict -> BLOCKED, never guessed. **Terminal rejected slot (review round):**
a rejected promotion is a human governance record — the add is BLOCKED with the prior
decision named; it is never silently reused, reopened as a side effect, or bypassed via a
parallel subject (explicit ADR-0045 reopen is the path back). A create whose slug is already
owned by a DIFFERENT typed id blocks (`slug_collision`) before any write. After a successful
add, `wiki/index.md` is rebuilt (producer contract); the audit filename keys on the node's
promote-slot review id even for a mention-only add (one deterministic key per node, whether
or not an item was filed — `promote_resolution: null` marks the mention-only case).
"""
...
_FAMILY = frozenset({"concept", "entity", "person", "organization", "project"})
_ENTITY_FAMILY = frozenset({"entity", "person", "organization", "project"})
_MAX_DESCRIPTION = 2000

# Node lifecycle states the add can work with; anything else (merged/rekeyed/hidden/…) blocks.
...
def add_candidate(
    ...
    *,
    ...
    `routed_subtype` (mention to the existing node + a change_entity_subtype item), or
    `blocked` with a `reason` (`invalid_*`, `unknown_source`, `cross_type_conflict`,
    `node_not_addable`, `promotion_previously_rejected` + the prior decision fields).
    """
    now = now or iso_now()
    wiki_dir, reviews_dir = Path(wiki_dir), Path(reviews_dir)
    if node_type not in _FAMILY:
        return _blocked("invalid_node_type", allowed=sorted(_FAMILY))
    title = concepts._WS.sub(" ", str(title or "")).strip()[:concepts._MAX_NAME]
    if not title:
        return _blocked("invalid_title")
    clean_aliases = concepts._clean_aliases(aliases or [])
    if description:  # canonical single-line prose (review round: whitespace collapsed at the boundary)
        description = concepts._WS.sub(" ", str(description)).strip()[:_MAX_DESCRIPTION] or None
    else:
        description = None
    ...
    existing = graph.find_node_by_candidate_ids(gconn, concepts._candidate_ids(title))
    routed_subtype: tuple[str, str] | None = None
    if existing is not None and existing["node_type"] != node_type:
        if not ({existing["node_type"], node_type} <= _ENTITY_FAMILY):
            # concept<->entity cross-family: a future change_node_type — never guessed (no writes).
            return _blocked("cross_type_conflict",
                            existing_node_id=existing["node_id"],
                            existing_type=existing["node_type"], requested_type=node_type)
        routed_subtype = (existing["node_type"], node_type)

    node = existing
    used_type = existing["node_type"] if existing is not None else node_type
    nid = existing["node_id"] if existing is not None else concepts.node_id(node_type, title)
    ...
    # --- producer writes (mirror extract_concepts' _emit, asserted_by=human, anchorless) ---
    node_created = node is None
    slug = concepts._slug(title) if node_created else node["slug"]
    if node_created:
        ...
        page_path = wiki_dir / NODE_DIR[used_type] / f"{slug}.md"
        if page_path.exists():
            fm = parse_frontmatter(page_path.read_text(encoding="utf-8", errors="replace"))
            if fm.get(concepts.ID_FIELD[used_type]) != nid:
                return _blocked("slug_collision", slug=slug,
                                existing_node_id=fm.get(concepts.ID_FIELD[used_type]))
        graph.upsert_node(gconn, node_id=nid, node_type=used_type, slug=slug,
    ...
    # withdraws its stale deprecation). The text hint seeds title/aliases only on a fresh page.
    concepts._recompose_node(gconn, node_id=nid, wiki_dir=wiki_dir, reviews_dir=reviews_dir,
    ...
    if node_created and description:
        ...
        page_path.write_text(render_concept_page({
            "node_type": used_type, "node_id": nid, "id_field": concepts.ID_FIELD[used_type],
    ...
    # The Source page's Mentioned sections are a graph projection — re-render so the human
    ...
    if routed_subtype is not None:
        reviews.create_review_item(
            reviews_dir, review_type="change_entity_subtype",
            subject={"node_id": nid, "to_type": routed_subtype[1]},
            proposal={"to_type": routed_subtype[1]},
            context={"source_id": source_id, "name": title,
                     "from_type": routed_subtype[0]}, now=now)
    ...
    if needs_promotion:
        reviews.create_review_item(
            ...
            subject={"node_id": nid},
            proposal={"to_status": "active", "name": title, "node_type": used_type}, now=now)
    ...
    (audit / f"{rid}-human-added-{uuid.uuid4().hex[:8]}.json").write_text(json.dumps({
        ...
        "source_id": source_id, "node_id": nid, "node_type": used_type,
        ...
        "routed_subtype": (
            {"from_type": routed_subtype[0], "to_type": routed_subtype[1]}
            if routed_subtype else None),
    ...
    index_rebuilt = concepts._rebuild_index(Path(root))

    outcome = ("routed_subtype" if routed_subtype
               else "created" if node_created
               else "mention_added_active" if not needs_promotion
               else "mention_added")
    return {"outcome": outcome, "node_id": nid, "node_type": used_type, "review_id": rid,
