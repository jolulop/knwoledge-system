...
def _effect_hide_synthesis(item: dict[str, Any], gconn: Any, wiki_dir: Path | None) -> tuple[str, list[str]]:
    # ADR-0049: synthesis hide. EFFECTED iff page AND graph node hidden (+ review_status approved); a partial
    # live hide (page XOR graph hidden, or both hidden + review pending) -> UNKNOWN partial_hide_state (NOT
    # reopen-safe); else PENDING_APPLY (+ synthesis_not_active when the target isn't active — hide is
    # active-only). Identical shape to _effect_hide_claim/_effect_hide_semantic, keyed on hidden-ness.
    status = item.get("status")
    if status not in ("approved", "rejected"):
        return PENDING_APPLY, []
    if status == "rejected":
    ...
    if page_hidden or graph_hidden:
        return UNKNOWN, ["partial_hide_state"]       # partial (incl. both hidden + review pending)
    warnings = [] if node["status"] == "active" else ["synthesis_not_active"]
    return PENDING_APPLY, warnings


def _effect_unhide_synthesis(item: dict[str, Any], gconn: Any,
                             wiki_dir: Path | None) -> tuple[str, list[str]]:
    # ADR-0049: synthesis unhide. EFFECTED iff page AND graph are NOT `hidden` (the operator hide is cleared;
    # the restored status is `active`, OR `evidence_hidden` when a supporting claim is still hidden — decision
    # 10. Either way it's no longer operator-`hidden`, so the unhide IS effected). PENDING_APPLY iff still
    # fully `hidden` (reopenable); a partial live unhide -> UNKNOWN partial_unhide_state. Keyed on
    # hidden-ness (mirrors _effect_unhide_claim).
    status = item.get("status")
    if status not in ("approved", "rejected"):
        return PENDING_APPLY, []
    if status == "rejected":
    ...
    graph_hidden = node["status"] == "hidden"
    if not page_hidden and not graph_hidden:
        return EFFECTED, []                          # no longer hidden (active)
    if page_hidden and graph_hidden:
        return PENDING_APPLY, []                     # still fully hidden -> reopenable
    return UNKNOWN, ["partial_unhide_state"]


def _preview_synthesis_visibility(item: dict[str, Any], *, gconn: Any, wiki_dir: Path | None,
                                  hide: bool) -> dict[str, Any]:
    subj = item.get("subject") or {}
    page = subj.get("page")
    out = _scaffold(item)
    ...
    out["affected_paths"] = [page] if page else []
    out["proposed_status"] = "hidden" if hide else "active"
    out["proposed_action"] = ("hide synthesis (active -> hidden)" if hide
                              else "unhide synthesis (hidden -> active)")
    out["summary"] = f"{'Hide' if hide else 'Unhide'} synthesis {page}."
    fm = _page_frontmatter(wiki_dir, page)
    out["current_status"] = fm.get("status") if fm else None
    out["details"] = {"node_type": "synthesis"}
    effect = _effect_hide_synthesis if hide else _effect_unhide_synthesis
    rtype = "hide_synthesis" if hide else "unhide_synthesis"
    effect_status, warnings = effect(item, gconn, wiki_dir)
    out["apply"] = _apply_supported(EXECUTOR_BY_TYPE[rtype], effect_status, warnings)
    return out


def preview_hide_synthesis(item: dict[str, Any], *, gconn: Any, wiki_dir: Path | None) -> dict[str, Any]:
    return _preview_synthesis_visibility(item, gconn=gconn, wiki_dir=wiki_dir, hide=True)


def preview_unhide_synthesis(item: dict[str, Any], *, gconn: Any, wiki_dir: Path | None) -> dict[str, Any]:
