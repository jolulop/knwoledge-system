...
from app.workers import concepts, extract, human_add, intake, promote, reviews, wiki
...
class RoutingAdapter:
    ...
    def __init__(self, routes: dict[str, dict], default=None):
        self._routes = routes
        self._default = default or {"concepts": [], "entities": []}
    ...
    def parse(self, messages, schema, model_id, *, max_tokens):
        ...
        for marker, payload in self._routes.items():
            if marker in text:
                return {"concepts": [dict(c) for c in payload.get("concepts", [])],
...
def test_index_and_multi_source_candidate_attribution(tmp_path):
    ...
    _extract(tmp_path, RoutingAdapter({
        "alpha": {"concepts": [_cpt("Shared"), _cpt("Only A")]},
        "beta": {"concepts": [_cpt("Shared"), _cpt("Only B")]},
    }))
    sids = _sids(tmp_path)
...
def test_retired_section_h_predicate(tmp_path):
    _build(tmp_path, {"a.md": "alpha body", "b.md": "beta body"})
    _extract(tmp_path, RoutingAdapter({
        "alpha": {"concepts": [_cpt("Gone"), _cpt("Both Gone")]},
        "beta": {"concepts": [_cpt("Both Gone"), _cpt("Keeper B")]},
    }))
    sids = _sids(tmp_path)
    a, b = sids["a.md"], sids["b.md"]
    # Re-extract BOTH sources without the old concepts: Gone (H={a}) and Both Gone (H={a,b}).
    _rewrite_md(tmp_path, a, "alpha second body")
    _rewrite_md(tmp_path, b, "beta second body")
    _extract(tmp_path, RoutingAdapter({
        "alpha": {"concepts": [_cpt("New A")]},
        "beta": {"concepts": [_cpt("Keeper B")]},
    }))
    ...
    both_gone_id = concepts.node_id("concept", "Both Gone")
    ...
    graph.upsert_node(gconn, node_id="cpt_feedfeedfeedfeed", node_type="concept",
                      slug="no-history", status="deprecated_candidate")
    gconn.close()
    reviews.create_review_item(
        tmp_path / "reviews", review_type="deprecate_wiki_page",
        subject={"node_id": "cpt_feedfeedfeedfeed", "page": "Concepts/no-history.md"},
    ...
    assert all(r["node_id"] != "cpt_feedfeedfeedfeed"
...
def test_batch_decide_partial_skip_and_drafts(client, tmp_path):
    ...
    _extract(tmp_path, RoutingAdapter({"alpha": {"concepts": [
        _cpt("Approve Me"), _cpt("Reject Me"), _cpt("Defer Me"),
        _cpt("Untouched"), _cpt("Predecided")]}}))
...
def test_json_approve_rejects_bad_amendments(client, tmp_path):
    _build(tmp_path, {"a.md": "alpha body"})
    _extract(tmp_path, RoutingAdapter({"alpha": {"concepts": [_cpt("Solo")]}}))
...
def test_amendments_apply_frozen_id_slug_move_and_description(tmp_path):
    _build(tmp_path, {"a.md": "alpha body"})
    _extract(tmp_path, RoutingAdapter({"alpha": {"concepts": [_cpt("Old Name")]}}))
    a = _sids(tmp_path)["a.md"]
    nid = concepts.node_id("concept", "Old Name")
    ...
    old_page = tmp_path / "wiki" / "Concepts" / "old-name.md"
    new_page = tmp_path / "wiki" / "Concepts" / "new-name.md"
    ...
    assert fm["concept_id"] == nid                                  # frozen id, never re-hashed
    ...
    assert "[[Concepts/new-name" in src_page and "old-name" not in src_page
...
def test_amended_slug_collision_skips_with_reason(tmp_path):
    _build(tmp_path, {"a.md": "alpha body"})
    _extract(tmp_path, RoutingAdapter({"alpha": {"concepts": [_cpt("First"), _cpt("Second")]}}))
    reviews.resolve_review_item(
    ...
    fm = parse_frontmatter(
        (tmp_path / "wiki" / "Concepts" / "second.md").read_text(encoding="utf-8"))
...
def test_human_add_matrix(tmp_path):
    ...
    _extract(tmp_path, RoutingAdapter({"alpha": {
        "concepts": [_cpt("Existing")],
        "entities": [{"name": "Jane Doe", "entity_type": "person", "aliases": []}]}}))
    a = _sids(tmp_path)["a.md"]
    rdir = tmp_path / "reviews"

    # (1) brand-new candidate: producer writes + pre-approved promote + purpose-named audit.
    out = _human_add(tmp_path, source_id=a, node_type="concept", title="Brand New",
    ...
    gconn.close()
    page = tmp_path / "wiki" / "Concepts" / "brand-new.md"
    ...
    out2 = _human_add(tmp_path, source_id=a, node_type="concept", title="Existing")
    assert out2["outcome"] == "mention_added"
    assert out2["node_id"] == concepts.node_id("concept", "Existing")
    assert (rdir / "approved" / f"{out2['review_id']}.json").exists()

    # (3) subtype collision (entity family): mention routed + change_entity_subtype filed.
    out3 = _human_add(tmp_path, source_id=a, node_type="organization", title="Jane Doe")
    assert out3["outcome"] == "routed_subtype" and out3["node_type"] == "person"
    retype = [json.loads(p.read_text(encoding="utf-8"))
              for p in (rdir / "pending").glob("*.json")]
    retype = [r for r in retype if r["type"] == "change_entity_subtype"
              and r["subject"]["node_id"] == out3["node_id"]]
    assert len(retype) == 1 and retype[0]["context"]["source_id"] == a
    assert retype[0]["subject"]["to_type"] == "organization"

    # (4) cross-family conflict: blocked, nothing written.
    before = len(list((rdir / "audit_log").glob("*-human-added-*.json")))
    out4 = _human_add(tmp_path, source_id=a, node_type="concept", title="Jane Doe")
    assert out4 == {"outcome": "blocked", "reason": "cross_type_conflict",
                    "existing_node_id": out3["node_id"], "existing_type": "person",
    ...
    _human_add(tmp_path, source_id=a, node_type="concept", title="Doomed")  # created+approved
    ...
    reviews.resolve_review_item(rdir, doomed_rid, decision="rejected", decided_by="human",
                                note="not wanted")
    out5 = _human_add(tmp_path, source_id=a, node_type="concept", title="Doomed")
    ...
    assert _human_add(tmp_path, source_id="src_0000000000000000", node_type="concept",
...
def test_human_add_already_active_mention_only(tmp_path):
    _build(tmp_path, {"a.md": "alpha body", "b.md": "beta body"})
    _extract(tmp_path, RoutingAdapter({"alpha": {"concepts": [_cpt("Hot Topic")]}}))
    sids = _sids(tmp_path)
    a, b = sids["a.md"], sids["b.md"]
    ...
    promote.promote_candidates(tmp_path, jobs_db=tmp_path / "db" / "jobs.sqlite",
                               rebuild_index=False)
    out = _human_add(tmp_path, source_id=b, node_type="concept", title="Hot Topic")
...
def test_batch_rejects_non_visible_and_global_rids(client, tmp_path):
    _build(tmp_path, {"a.md": "alpha body", "b.md": "beta body"})
    _extract(tmp_path, RoutingAdapter({
        "alpha": {"concepts": [_cpt("Mine")]},
        "beta": {"concepts": [_cpt("Foreign")]},
    }))
    sids = _sids(tmp_path)
    a = sids["a.md"]
    ...
    global_rid = reviews.create_review_item(         # a global type: never source-attributable
        tmp_path / "reviews", review_type="merge_entities",
        subject={"survivor_node_id": "per_1111111111111111",
                 "absorbed_node_id": "per_2222222222222222"},
...
def test_human_add_slug_collision_blocks_before_any_write(tmp_path):
    _build(tmp_path, {"a.md": "alpha body"})
    _extract(tmp_path, RoutingAdapter({"alpha": {"concepts": [_cpt("Foo Bar")]}}))
    a = _sids(tmp_path)["a.md"]
    # "Foo-Bar" normalizes to a DIFFERENT name-hash than "Foo Bar" but the SAME slug.
    new_id = concepts.node_id("concept", "Foo-Bar")
    assert new_id != concepts.node_id("concept", "Foo Bar")
    assert concepts._slug("Foo-Bar") == concepts._slug("Foo Bar") == "foo-bar"
    rdir = tmp_path / "reviews"
    audits_before = len(list((rdir / "audit_log").glob("*-human-added-*.json")))

    out = _human_add(tmp_path, source_id=a, node_type="concept", title="Foo-Bar")
    assert out["outcome"] == "blocked" and out["reason"] == "slug_collision"
    assert out["existing_node_id"] == concepts.node_id("concept", "Foo Bar")
    ...
    fm = parse_frontmatter(
        (tmp_path / "wiki" / "Concepts" / "foo-bar.md").read_text(encoding="utf-8"))
    assert fm["concept_id"] == concepts.node_id("concept", "Foo Bar")
...
def test_human_add_rebuilds_wiki_index(tmp_path):
    ...
    out = _human_add(tmp_path, source_id=a, node_type="concept", title="Indexed Fresh")
...
def test_source_screen_escapes_untrusted_text(client, tmp_path):
    ...
    _build(tmp_path, {"a.md": "alpha body"})
    _extract(tmp_path, RoutingAdapter({"alpha": {"concepts": [
        {"name": f"Evil {hostile}", "aliases": [f"x' onerror='{hostile}"]}]}}))
    a = _sids(tmp_path)["a.md"]
    _human_add(tmp_path, source_id=a, node_type="concept", title="Described",
