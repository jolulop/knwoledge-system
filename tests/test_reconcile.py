...
from app.workers import extract, intake, items, reconcile, reviews
...
class FakeAdapter:
    ...
    def parse(self, messages, schema, model_id, *, max_tokens):
        return {"items": [dict(i) for i in self._payload.get("items", [])]}
...
def _extract(tmp_path, item_names):
    adapter = FakeAdapter({"items": [{"name": n, "item_type": "method_technique", "aliases": []}
                                     for n in item_names]})
...
def _deprecate_rid(name):
    return reviews.review_id("deprecate_wiki_page",
                             {"node_id": items.node_id(name),
                              "page": f"Items/{items._slug(name)}.md"})


def _write_item_page(tmp_path, slug, node_id, status):
    page_dir = tmp_path / "wiki" / "Items"
...
def _promote_item(node="itm_x", status="pending"):
    return {"type": "promote_candidate_node", "review_id": "rev_x", "status": status,
            "subject": {"node_id": node}, "proposal": {"to_status": "active"}, "context": {}}


def _deprecate_item(reason=None, reason_code=None, node_type="item", status="pending"):
    ...
    return {"type": "deprecate_wiki_page", "review_id": "rev_y", "status": status,
            "subject": {"node_id": "itm_x", "page": "Items/x.md"},
...
def test_decision_promote_matrix():
    ...
    # `rekeyed` is retired by ADR-0059 (retype never kills an id) — merged is the only
    # identity tombstone left; the reason NAME is kept for audit continuity.
    assert d(_promote_item(), graph_status="merged", page_status="merged",
             active_source_count=0) == reconcile.REASON_MISSING_OR_REKEYED
...
def test_dir_id_field_parity():
    # The sweep's local dir->id-field map must track wiki_render.NODE_DIR (kept local to
    # avoid the items->reconcile import cycle). One item family, one typed id field.
    assert reconcile._DIR_ID_FIELD == {NODE_DIR["item"]: "item_id"}
...
def test_hook_tombstone_withdraws_promote_and_files_reason_code(tmp_path):
    ...
    # The live item's promote is untouched.
...
def test_hook_leaves_foreign_reason_deprecate_on_tombstone(tmp_path):
    ...
    dep_rid = reviews.create_review_item(
        ...
        subject={"node_id": items.node_id("Old"), "page": "Items/old.md"},
...
def _eligible_promote(tmp_path, node_id="itm_feedfeedfeedfeed"):
    return reviews.create_review_item(
        tmp_path / "reviews", review_type="promote_candidate_node",
        subject={"node_id": node_id}, proposal={"to_status": "active", "item_type": "method_technique"})
...
def test_sweep_refuses_schema_version_mismatch(tmp_path):
    ...
    graph.init_db(tmp_path / "db" / "graph.sqlite")
    gconn = graph.connect(tmp_path / "db" / "graph.sqlite")
    graph.upsert_node(gconn, node_id="itm_aaaaaaaaaaaaaaaa", node_type="item", item_type="method_technique",
...
def test_sweep_refuses_on_graph_wiki_drift(tmp_path):
    ...
    _write_item_page(tmp_path, "page-only", "itm_bbbbbbbbbbbbbbbb", "candidate")  # no mirror
    graph.upsert_node(gconn, node_id="itm_cccccccccccccccc", node_type="item", item_type="method_technique",
                      slug="disagrees", status="active")
    _write_item_page(tmp_path, "disagrees", "itm_cccccccccccccccc", "candidate")
    rids = [_eligible_promote(tmp_path, node_id=nid)
            for nid in ("itm_aaaaaaaaaaaaaaaa", "itm_bbbbbbbbbbbbbbbb", "itm_cccccccccccccccc")]
    counts = _sweep(tmp_path, gconn)
    gconn.close()
...
def test_sweep_skips_terminal_and_malformed_pending_files(tmp_path):
    ...
    _write_item_page(tmp_path, "unrelated", "itm_aaaaaaaaaaaaaaaa", "candidate")
    ...
    terminal = dict(_promote_item(node="itm_9999999999999999", status="approved"),
...
def test_sweep_matrix_legacy_shim_and_idempotence(tmp_path):
    ...
    stale_new = reviews.create_review_item(
        ...
        subject={"node_id": items.node_id("New")},
        proposal={"to_status": "active", "item_type": "method_technique"})
    ghost = reviews.create_review_item(
        reviews_dir, review_type="promote_candidate_node",
        subject={"node_id": "itm_0000000000000000"},
        proposal={"to_status": "active", "item_type": "method_technique"})
    gconn = graph.connect(tmp_path / "db" / "graph.sqlite")
    graph.upsert_node(gconn, node_id="itm_1111111111111111", node_type="item", item_type="method_technique",
                      slug="already-active", status="active")
    _write_item_page(tmp_path, "already-active", "itm_1111111111111111", "active")
    active_rid = reviews.create_review_item(
        reviews_dir, review_type="promote_candidate_node",
        subject={"node_id": "itm_1111111111111111"},
        proposal={"to_status": "active", "item_type": "method_technique"})
    ...
    lint_rid = reviews.create_review_item(
        ...
        subject={"node_id": items.node_id("Keeper"), "page": "Items/keeper.md"},
        proposal={"to_status": "deprecated_candidate",
    ...
    approved_rid = reviews.create_review_item(
        reviews_dir, review_type="promote_candidate_node",
        subject={"node_id": "itm_2222222222222222"},
        proposal={"to_status": "active", "item_type": "method_technique"})
