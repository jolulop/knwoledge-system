...
from app.workers import duplicates, items
...
def _seed_node(tmp_path: Path, gconn, *, node_id: str, slug: str, node_type: str = "item",
               title: str | None = None, status: str = "active") -> Path:
    """A graph node + its rendered page (so the executor can re-render it)."""
    item_type = "product_tool_platform" if node_type == "item" else None
...
def test_approve_creates_canonical_active_edge_and_projects_both_pages(tmp_path):
    ...
    # ids chosen so itm_a < itm_b lexically
    pa = _seed_node(tmp_path, gconn, node_id="itm_aaaaaaaaaaaaaaaa", slug="alpha", title="Alpha")
    pb = _seed_node(tmp_path, gconn, node_id="itm_bbbbbbbbbbbbbbbb", slug="bravo", title="Bravo")
    gconn.commit()
    _approve(tmp_path, _dup_item("itm_aaaaaaaaaaaaaaaa", "itm_bbbbbbbbbbbbbbbb"))
...
def test_steady_state_reapply_is_a_true_noop(tmp_path):
    ...
    graph.init_db(gdb)
    gconn = graph.connect(gdb)
    pa = _seed_node(tmp_path, gconn, node_id="itm_aaaaaaaaaaaaaaaa", slug="alpha")
    _seed_node(tmp_path, gconn, node_id="itm_bbbbbbbbbbbbbbbb", slug="bravo")
    gconn.commit()
    _approve(tmp_path, _dup_item("itm_aaaaaaaaaaaaaaaa", "itm_bbbbbbbbbbbbbbbb"))
    wiki = tmp_path / "wiki"
...
def test_stale_projection_is_normalized_not_applied(tmp_path):
    ...
    pa = _seed_node(tmp_path, gconn, node_id="itm_aaaaaaaaaaaaaaaa", slug="alpha")
    _seed_node(tmp_path, gconn, node_id="itm_bbbbbbbbbbbbbbbb", slug="bravo")
    graph.upsert_assertion(gconn, src_id="itm_aaaaaaaaaaaaaaaa", dst_id="itm_bbbbbbbbbbbbbbbb",
...
def test_recompose_returns_unchanged_when_no_write(tmp_path):
    ...
    graph.init_db(gdb)
    gconn = graph.connect(gdb)
    page = _seed_node(tmp_path, gconn, node_id="itm_aaaaaaaaaaaaaaaa", slug="alpha")
    gconn.commit()
    # first recompose normalizes to canonical form
    items.recompose_semantic_node_page(gconn, node_id="itm_aaaaaaaaaaaaaaaa",
                                          wiki_dir=tmp_path / "wiki", status="active", review_status="none")
    before = page.read_text()
    res = items.recompose_semantic_node_page(gconn, node_id="itm_aaaaaaaaaaaaaaaa",
...
@pytest.mark.parametrize("subject,reason", [
    ({"node_ids": ["itm_aaaaaaaaaaaaaaaa"]}, "malformed_subject"),         # wrong length
    ({"node_ids": "itm_aaaaaaaaaaaaaaaa"}, "malformed_subject"),           # not a list
    ({}, "malformed_subject"),                                             # missing
    ({"node_ids": ["itm_aaaaaaaaaaaaaaaa", "itm_aaaaaaaaaaaaaaaa"]}, "self_duplicate"),
    ({"node_ids": ["itm_aaaaaaaaaaaaaaaa", "../escape"]}, "invalid_node_id"),
])
def test_scope_guard_skips(tmp_path, subject, reason):
    ...
    graph.init_db(gdb)
    gconn = graph.connect(gdb)
    _seed_node(tmp_path, gconn, node_id="itm_aaaaaaaaaaaaaaaa", slug="alpha")
    gconn.commit()
...
def test_node_missing_and_type_mismatch_skips(tmp_path):
    ...
    gconn = graph.connect(gdb)
    _seed_node(tmp_path, gconn, node_id="itm_aaaaaaaaaaaaaaaa", slug="alpha")
    graph.upsert_node(gconn, node_id="clm_cccccccccccccccc", node_type="claim", slug="gamma",
                      status="active")
    gconn.commit()
    # node_missing: itm_d not in graph
    _approve(tmp_path, _dup_item("itm_aaaaaaaaaaaaaaaa", "itm_dddddddddddddddd", rid="rev_nm"))
    # type_mismatch: item vs claim (cross item_type is ALLOWED now — same node_type "item")
    _approve(tmp_path, _dup_item("itm_aaaaaaaaaaaaaaaa", "clm_cccccccccccccccc", rid="rev_tm"))
    res = duplicates.apply_marked_duplicates(gconn, tmp_path / "reviews", wiki_dir=tmp_path / "wiki")
    gconn.commit()
...
def test_preview_projector_resolves_paths_and_supported(tmp_path):
    ...
    _seed_node(tmp_path, gconn, node_id="itm_bbbbbbbbbbbbbbbb", slug="bravo")
    gconn.commit()
    item = _dup_item("itm_aaaaaaaaaaaaaaaa", "itm_bbbbbbbbbbbbbbbb")
    ...
    assert out["node_ids"] == ["itm_aaaaaaaaaaaaaaaa", "itm_bbbbbbbbbbbbbbbb"]
    assert set(out["affected_paths"]) == {"Items/alpha.md", "Items/bravo.md"}
    assert out["warnings"] == []
...
def test_preview_projector_warns_already_duplicated_and_type_mismatch(tmp_path):
    ...
    dup = review_read.preview_mark_semantic_duplicate(
        _dup_item("itm_aaaaaaaaaaaaaaaa", "itm_bbbbbbbbbbbbbbbb"), gconn=gconn, wiki_dir=tmp_path / "wiki")
    ...
    tm = review_read.preview_mark_semantic_duplicate(
        _dup_item("clm_cccccccccccccccc", "itm_aaaaaaaaaaaaaaaa"), gconn=gconn, wiki_dir=tmp_path / "wiki")
...
def test_preview_projector_invalid_and_malformed(tmp_path):
    ...
    inv = review_read.preview_mark_semantic_duplicate(
        {"review_id": "r", "type": "mark_semantic_duplicate", "status": "approved",
         "subject": {"node_ids": ["itm_aaaaaaaaaaaaaaaa", "../escape"]}},
    ...
    mal = review_read.preview_mark_semantic_duplicate(
        {"review_id": "r", "type": "mark_semantic_duplicate", "status": "approved",
         "subject": {"node_ids": "itm_aaaaaaaaaaaaaaaa"}},
        gconn=gconn, wiki_dir=tmp_path / "wiki")
...
def test_validate_graph_rejects_reversed_duplicates(tmp_path):
    ...
    graph.init_db(gdb)
    gconn = graph.connect(gdb)
    graph.upsert_node(gconn, node_id="itm_aaaaaaaaaaaaaaaa", node_type="item", slug="a",
                      status="active", item_type="model")
    graph.upsert_node(gconn, node_id="itm_bbbbbbbbbbbbbbbb", node_type="item", slug="b",
                      status="active", item_type="model")
    ...
    gconn.execute("INSERT INTO edges (edge_id, src_id, dst_id, edge_type, status, asserted_by, "
                  "created_at) VALUES ('e1','itm_bbbbbbbbbbbbbbbb','itm_aaaaaaaaaaaaaaaa',"
...
def test_validate_projection_both_directions(tmp_path):
    ...
    gconn = graph.connect(gdb)
    for nid in ("itm_aaaaaaaaaaaaaaaa", "itm_bbbbbbbbbbbbbbbb"):
        items.recompose_semantic_node_page(gconn, node_id=nid, wiki_dir=tmp_path / "wiki",
...
def test_apply_and_dry_run_end_to_end(client, tmp_path):
    ...
    assert "## Duplicates" not in (tmp_path / "wiki/Items/alpha.md").read_text()  # not yet applied

    # real apply lands it
    applied = client.post("/reviews/apply").json()
    assert applied["summary"]["duplicates"]["applied"] == 1
    assert "## Duplicates" in (tmp_path / "wiki/Items/alpha.md").read_text()
