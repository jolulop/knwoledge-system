"""ADR-0050/0059 identity surgery: knowledge-item merge (merge_items), forward-only.
...
withdraw unresolved B-subjects, audit. Pre-write BLOCK gates; never partial-apply. ADR-0059: both
endpoints are node_type "item" by construction; the survivor keeps its OWN item_type even when the
absorbed one differs (the old cross-type-merge deferral dissolved with typed prefixes).
"""
...
from app.workers import items, merges
from app.workers.wiki_render import parse_frontmatter, render_item_page

A = items.node_id("Alpha")
B = items.node_id("Beta")
...
def _item(tmp_path, conn, nid, name, *, status="active", aliases=(), item_type=ITEM_TYPE):
    slug = items._slug(name)
    ...
    page.write_text(render_item_page({
        "node_id": nid, "item_type": item_type, "title": name,
...
def _mention(conn, sid, nid, *, status="active", span=None, review_id=None):
...
def _approve(tmp_path, *, survivor=A, absorbed=B, rtype="merge_items", rid="rev_m"):
...
def _fm(tmp_path, slug):
    return parse_frontmatter((tmp_path / "wiki" / "Items" / f"{slug}.md").read_text())
...
def test_merge_tombstones_b_and_keeps_a_active(tmp_path):
    ...
    a_slug = _item(tmp_path, conn, A, "Alpha", aliases=["A1"])
    b_slug = _item(tmp_path, conn, B, "Beta", aliases=["B1"])
    _approve(tmp_path)
    ...
    assert fm_b["item_type"] == ITEM_TYPE                         # tombstone carries item_type
    assert "Merged into" in (tmp_path / "wiki" / "Items" / f"{b_slug}.md").read_text()
...
def test_cross_item_type_merge_survivor_keeps_own_type(tmp_path):
    # ADR-0059: both nodes are the same structural type by construction, so a merge across
    # DIFFERENT item_types is allowed and the survivor keeps its OWN classification — the old
    # cross-type block (typed prefixes) no longer exists.
    conn = _graph(tmp_path)
    a_slug = _item(tmp_path, conn, A, "Alpha", item_type="product_tool_platform")
    b_slug = _item(tmp_path, conn, B, "Beta", item_type="model")
    _approve(tmp_path)
    res = _apply(tmp_path, conn)
    assert res["applied"] == 1 and res["skipped"] == []
    assert _fm(tmp_path, a_slug)["item_type"] == "product_tool_platform"   # survivor's OWN type
    assert graph.get_node(conn, A)["item_type"] == "product_tool_platform"
    fm_b = _fm(tmp_path, b_slug)
    assert fm_b["status"] == "merged" and fm_b["item_type"] == "model"     # tombstone keeps B's type
    assert graph.get_node(conn, B)["item_type"] == "model"
    conn.close()


def test_merge_is_idempotent_on_reapply(tmp_path):
    conn = _graph(tmp_path)
    _item(tmp_path, conn, A, "Alpha")
    _item(tmp_path, conn, B, "Beta")
    _approve(tmp_path)
...
def test_resurrect_proposed_target_collision(tmp_path):
    ...
    (tmp_path / "reviews" / "pending" / "rev_prop.json").write_text(json.dumps({
        ...
        "subject": {"node_id": "itm_ffffffffffffff00"}}), encoding="utf-8")
    _mention(conn, SID, B, span=(0, 4))
    _approve(tmp_path)
    res = _apply(tmp_path, conn)
...
def test_collapse_and_resurrect_still_re_render_the_source(tmp_path):
    ...
    # stale [[Items/B]] otherwise) — affected_sources covers EVERY action, not just repoint.
    conn = _graph(tmp_path)
    _item(tmp_path, conn, A, "Alpha")
    _item(tmp_path, conn, B, "Beta")
...
def test_duplicates_collapse_re_renders_partner_dropping_b(tmp_path):
    C = items.node_id("Gamma")
    conn = _graph(tmp_path)
    _item(tmp_path, conn, A, "Alpha")
    _item(tmp_path, conn, B, "Beta")
    c_slug = _item(tmp_path, conn, C, "Gamma")
    ...
    items.recompose_semantic_node_page(conn, node_id=C, wiki_dir=tmp_path / "wiki",
                                       status="active", review_status="none")
    assert "beta" in (tmp_path / "wiki" / "Items" / f"{c_slug}.md").read_text()
    _approve(tmp_path)
    _apply(tmp_path, conn)
...
def test_tombstone_carries_merge_audit_fields(tmp_path):
    conn = _graph(tmp_path)
    _item(tmp_path, conn, A, "Alpha")
    b_slug = _item(tmp_path, conn, B, "Beta")
    _approve(tmp_path)
    _apply(tmp_path, conn)
...
@pytest.mark.parametrize("corrupt", ["itm_deaddeaddeaddead", "_WRONG_TYPE_", "_SELF_"])
def test_validator_rejects_bad_merged_into(tmp_path, corrupt):
    # non-indexed survivor, wrong-type survivor (B is an item; point at a claim), and self pointer.
    ...
    if corrupt == "_WRONG_TYPE_":                                # wrong-type: a claim survivor
        corrupt = "clm_deadbeefdeadbeef"
        conn = graph.connect(tmp_path / "db" / "graph.sqlite")
        graph.upsert_node(conn, node_id=corrupt, node_type="claim", slug="delta", status="active")
        conn.commit()
        conn.close()
    target = B if corrupt == "_SELF_" else corrupt
    page = tmp_path / "wiki" / "Items" / f"{b_slug}.md"
...
def test_validator_rejects_inactive_survivor(tmp_path):
    ...
    items.recompose_semantic_node_page(conn, node_id=A, wiki_dir=tmp_path / "wiki",
...
def test_partial_merge_state_is_unknown(tmp_path):
    ...
    conn = _graph(tmp_path)
    _item(tmp_path, conn, A, "Alpha")
    _item(tmp_path, conn, B, "Beta")                              # page status active
    graph.upsert_node(conn, node_id=B, node_type="item", slug="beta", status="merged")  # graph only
    conn.commit()
    item = {"type": "merge_items", "status": "approved",
...
@pytest.mark.parametrize("survivor,absorbed,reason", [
    (A, A, "self_merge"),
    (A, "itm_ffffffffffffffff", "node_missing"),
])
...
def test_type_mismatch_and_out_of_scope_skip(tmp_path):
    conn = _graph(tmp_path)
    _item(tmp_path, conn, A, "Alpha")
    # absorbed is a claim (different structural node_type) -> type_mismatch for merge_items
    graph.upsert_node(conn, node_id="clm_aaaaaaaaaaaaaaaa", node_type="claim", slug="clm_a",
                      status="active")
    # both endpoints claims (same structural type, but not items) -> out_of_scope
    graph.upsert_node(conn, node_id="clm_bbbbbbbbbbbbbbbb", node_type="claim", slug="clm_b",
                      status="active")
    conn.commit()
    _approve(tmp_path, absorbed="clm_aaaaaaaaaaaaaaaa")
    _approve(tmp_path, survivor="clm_aaaaaaaaaaaaaaaa", absorbed="clm_bbbbbbbbbbbbbbbb", rid="rev_o")
    res = _apply(tmp_path, conn)
    reasons = {s["review_id"]: s["reason"] for s in res["skipped"]}
    assert reasons == {"rev_m": "type_mismatch", "rev_o": "out_of_scope"}
    conn.close()


def test_item_type_missing_is_typed_skip(tmp_path):
    # ADR-0059: a drifted node whose page AND graph mirror both lack a valid item_type must be a
    # typed skip, never a mid-apply raise from the graph write gate.
    conn = _graph(tmp_path)
    _item(tmp_path, conn, A, "Alpha")
    b_slug = _item(tmp_path, conn, B, "Beta")
    conn.execute("UPDATE nodes SET item_type = NULL WHERE node_id = ?", (B,))   # drifted mirror
    conn.commit()
    page = tmp_path / "wiki" / "Items" / f"{b_slug}.md"
    page.write_text(page.read_text().replace(f"item_type: {ITEM_TYPE}\n", ""), encoding="utf-8")
    _approve(tmp_path)
    res = _apply(tmp_path, conn)
    assert res["applied"] == 0
    assert res["skipped"] == [{"review_id": "rev_m", "reason": "item_type_missing"}]
    assert graph.get_node(conn, B)["status"] == "active"         # no writes — B untouched
    conn.close()
...
def test_withdraws_pending_and_deferred_b_subjects(tmp_path):
    ...
    (pend / "d1.json").write_text(json.dumps({
        ...
        "subject": {"node_id": B, "page": f"Items/{b_slug}.md"}}), encoding="utf-8")
...
def test_approved_unapplied_references_absorbed_blocks(tmp_path):
    ...
    (tmp_path / "reviews" / "approved" / "hb.json").write_text(json.dumps({
        ...
        "subject": {"node_id": B, "page": f"Items/{b_slug}.md"},
        "proposal": {"to_status": "hidden"}, "context": {"node_type": "item"}}), encoding="utf-8")
    res = _apply(tmp_path, conn)
    assert res["applied"] == 0
    assert {"review_id": "rev_m", "reason": "approved_unapplied_references_absorbed"} in res["skipped"]
    conn.close()
...
def test_validator_rejects_active_edge_with_merged_endpoint(tmp_path):
    ...
    graph.upsert_node(conn, node_id=B, node_type="item", slug="beta", status="merged")
    conn.commit()
    conn.close()
