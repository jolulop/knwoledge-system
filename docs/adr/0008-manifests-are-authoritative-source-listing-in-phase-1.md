...
Phase 2 clarification: manifests are authoritative local runtime state, not committed project source. Git tracks the `raw/manifests/` directory placeholder, while generated `raw/manifests/*.json` files are ignored. This keeps source discovery reproducible on a given machine without treating local scan results, absolute paths, or user document inventory as portable repository content.
