...
The SQLite store under `db/graph.sqlite` that is authoritative for **relationships (edges)** — edges keyed on stable typed ids (`source_id`/`concept_id`/`entity_id`/`claim_id`), not slugs, drawn from the governed Build Spec §6.2 vocabulary. Node *metadata* (id/title/slug/status/aliases) stays owned by wiki frontmatter (ADR-0017/0021/0022); the graph's `nodes` table is a derived index rebuilt from pages, and the wikilink/backlink sections in pages are a derived projection of the graph's `active` edges — so rename/merge is an id-level redirect and backlinks stay synchronized by construction. Each edge carries `status` (proposed|active|rejected|superseded), provenance (`asserted_by`), confidence, and an evidence anchor; the projector renders only `active` edges, so an LLM- or prose-authored edge enters as a review-gated `proposed` candidate and never appears as a backlink until approved (ADR-0029, ADR-0030; lands in Phase 3.5b).
